...
function formatArgs(args, freeform) {
  var formattedArgs = [];
...
AbstractAnsibleCommand.prototype.exec = function(options) {
  ...
  //console.log('compileParams',this.compileParams());
   console.log('processOptions',processOptions);

  console.log('Executing Ansible Playbook Command:[%s] Params:[%s]',this.commandName(),this.compileParams())
...
var Playbook = function () {
  ...
  this.compileParams = function() {
    var result = [
      this.config.playbook.endsWith(".yml") ? this.config.playbook : this.config.playbook + ".yml"
    ];
